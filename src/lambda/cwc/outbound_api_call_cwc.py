...
def send_to_sqs(record):
    response = sqs_client.send_message(
        QueueUrl="https://sqs.us-east-1.amazonaws.com/030615279213/Emarsys-genesys-integration-sqs-cwc",
...
def lambda_handler(event, context):
    ...
    try:
        ...
        send_sns_notification(f'outbound_api_call_cwc an exception: {str(e)}',
                              '!!! outbound_api_call_cwc : Rate limit exceeded exception!!!')
            ...
            try:
                ...
                if error_body:
                    ...
                    if "rate limit exceeded" in str(error_message).lower():
                        send_sns_notification('outbound_api_call_cwc an exception', '!!! Rate limit exceeded exception!!!')
                        send_to_sqs_list.append(record)
                        send_to_sqs(data)
                    else:
    ...
    if payload["success"] or payload["failures"]:
        try:
            response = lambda_client.invoke(
                FunctionName='arn:aws:lambda:us-east-1:030615279213:function:outbound_data_updates_cwc',
