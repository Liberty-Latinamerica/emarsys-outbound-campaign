...
def send_to_sqs(record):
    response = sqs_client.send_message(
        QueueUrl="https://sqs.us-east-1.amazonaws.com/030615279213/emarsys-genesys-integration-sqs",
...
def lambda_handler(event, context):
    ...
    try:
        apiclient = PureCloudPlatformClientV2.api_client.ApiClient().get_client_credentials_token(
            '92edaaa5-a1b2-481b-bf84-61431422cdbe', 'll4884fL5N8_WoLXYjhOAUQdX4Gu6-PuasIMVUUMM2w')
        ...
        send_sns_notification(f'outbound_api_call an exception: {str(e)}',
                              '!!! outbound_api_call : Rate limit exceeded exception!!!')
            ...
            try:
                ...
                if error_body:
                    ...
                    if "rate limit exceeded" in str(error_message).lower():
                        send_sns_notification('outbound_api_call an exception', '!!! Rate limit exceeded exception!!!')
                        send_to_sqs_list.append(record)
                        send_to_sqs(data)
                    else:
    ...
    if payload["success"] or payload["failures"]:
        try:
            response = lambda_client.invoke(
                FunctionName='arn:aws:lambda:us-east-1:030615279213:function:outbound_data_updates_cwp',
