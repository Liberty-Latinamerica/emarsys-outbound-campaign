...
table1 = dynamodb.Table('emarsys_contactlist')

#CWP credentials
username = "lilac_pa005"
secret = "vHklhJ8doH42f9B98sda"
...
def lambda_handler(event, context):
    ...
    for item in response['Items']:
        toAddress.append(item.get('toAddress'))  # Use get() to handle missing keys gracefully
    ...
    # response_create_list= {'replyCode': 0, 'replyText': 'OK', 'data': {'id': 1928097223}}
    ...
    if response_create_list['data'] == '':
        ...
        print("updated contactList payload:", payload_update)
        print("old_update_contacts:", response_update_contacts.json())
    
    elif response_create_list['data'] != '':
        url_update_contacts = 'https://api.emarsys.net/api/v2/contact/'
        payload_update = {
            "key_id": "15",
        ...
        if response_create_list.get('message') == 'Rate limit exceeded':
